...
async def autosydd(client, file_details):
    try:
      #  remov_list = [ "", "", "", ""]
        ...
        remove_list = ["-Telly", "-GalaxyRG", "-TR", "-PSA", "-GalaxyRG265", "-GalaxyTV", "PIRO", "Eac3", "-BUAM", "St4LiLiN", "-HDHub4u.Tv", "HiQVE", "CG", "KMH", "Movie Bazar", "A2Movies", "FENiX", "VARY", "klandd", "KLandd",
        ...
        sydd = ['psa', 'sh3lby', 'Archie', 'Jo', 'Spidey', 'mkvcinemas', 'klandd', 'Telly', 'SH3LBY.mkv', 'bigil', 'YTS.MX', 'WADU', 'budgetbits', 'HDA', 'TR', 'primefix', 'GalaxyRG265', 'bone', 'Incursi0', 'StreliziA', 'ikaRos', 'lssjbroly', 'soan', 'KLandd', 'pahe', 'poke', 'galaxytv', 'galaxyrg', 'NazzY', 'VARYG', 'MICHAEL', 'FLUX', 'RAV1NE', '[YTS']
...
async def add_range(client, message):
    ...
    while added < limit and mid > 0:
        try:
            ...
            if file.file_size > 2000 * 1024 * 1024:
                await asyncio.sleep(random.choice(SYD))
                await client.copy_message(sydtg, message.chat.id, mid)
                await msg.delete()
                return
    ...
    await status.edit(f"✅ Added **{added} files** to DB")
    if not processing:
        processing = True
        asyncio.create_task(process_queue(client))
